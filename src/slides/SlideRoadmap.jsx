...
export default function SlideRoadmap() {
  ...
  return (
    <Slide
      bg={`
        radial-gradient(ellipse at 50% 70%, ${C.plum}18 0%, transparent 50%),
        ${C.dark}
      `}
    ...
    >
      <div style={{ maxWidth: 800, padding: "60px 40px", width: "100%" }}>
        ...
        <Stagger delay={0.15}>
          <h2 style={{ fontSize: "clamp(28px, 3.5vw, 44px)", fontWeight: 400, color: C.white, margin: "0 0 32px", lineHeight: 1.15, fontFamily: FONT_DISPLAY }}>
          ...
          {steps.map((s, i) => (
            ...
            >
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 15, fontWeight: 600, color: C.cream, fontFamily: FONT_BODY }}>{s.title}</div>
                  <div style={{ fontSize: 13, color: `${C.cream}77`, marginTop: 3, lineHeight: 1.5, fontFamily: FONT_BODY }}>{s.desc}</div>
                </div>
              </div>
            </motion.div>
          ))}
        ...
        <div style={{ marginTop: 16 }}>
          <DeepDive title="What your team needs to do" icon="&#128100;" color={C.plum}>
            <p><strong style={{ color: C.cream }}>Training:</strong> Single 2-hour session for approvers. The End Client Portal is designed for simplicity — average interaction time is 2–3 minutes per worker per week.</p>
            <p style={{ marginTop: 8 }}><strong style={{ color: C.cream }}>Day-to-day:</strong> Induct workers, approve timesheets, pay one invoice. Your clinical processes don't change. Admin burden reduces significantly.</p>
            <p style={{ marginTop: 8 }}><strong style={{ color: C.cream }}>CQC inspections:</strong> Audit packs generated on demand. Pre-written inspection responses provided.</p>
          </DeepDive>
        </div>
      </div>
    </Slide>
